...
public class PostingMediaUpdateJob extends StatelessJob<PostingMediaUpdateJob.Parameters> {
    ...
    public static final Logger log = LoggerFactory.getLogger(PostingMediaUpdateJob.class);

    @Inject
    private AttachmentIngest attachmentIngest;

    public PostingMediaUpdateJob() {
        retryCount(5, "PT10M");
    }

    @Override
    protected void setParameters(String parameters, ObjectMapper objectMapper) {
        this.parameters = objectMapper.readValue(parameters, PostingMediaUpdateJob.Parameters.class);
    }

    @Override
    protected void execute() throws Exception {
        attachmentIngest.updateMediaLocation(
            parameters.nodeName, parameters.postingId, parameters.remoteMediaNodeName, parameters.remoteMediaId,
            parameters.mediaId
        );
    ...
    protected String getJobDescription() {
        return super.getJobDescription() + " for remote media " + parameters.remoteMediaId + " from node "
            + parameters.remoteMediaNodeName + " attached to posting " + parameters.postingId + " at node "
            + parameters.nodeName;
    }
